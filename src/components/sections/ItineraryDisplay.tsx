...
export const ItineraryDisplay = ({ result, resultsRef }: Props) => {
  ...
  return (
    <div className="flex flex-col min-h-screen w-full"> 
        ...
        <div ref={resultsRef} style={{ width: '100%', maxWidth: '720px', marginTop: '2rem', position: 'relative', zIndex: 10 }}>
        {result && (
          <div style={{
                        ...
                        places?.length ? (
                          <div key={label}>
                            ...
                            {places.map((p: any, idx: number) => (
                              <div key={idx} style={{ fontSize: 13, color: 'rgba(255,255,255,0.9)', padding: '3px 0', display: 'flex', alignItems: 'flex-start', gap: 7 }}>
                                <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'rgba(255,255,255,0.5)', marginTop: 5, flexShrink: 0 }} />
                                {/* ✅ Safe render for both string and object */}
                                {typeof p === 'string' ? <span>{p}</span>
                                  : <span>{p.name}{p.detail && <span style={{ color: 'rgba(255,255,255,0.5)' }}> — {p.detail}</span>}</span>
                                }
                              </div>
                            ))}
                          </div>
